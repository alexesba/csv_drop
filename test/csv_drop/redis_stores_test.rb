...
class RedisProgressStoreTest < ActiveSupport::TestCase
  setup do
    @redis = FakeRedis.new
    @store = CsvDrop::Stores::RedisProgressStore.new(redis: @redis)
  end
...
class RedisSessionStoreTest < ActiveSupport::TestCase
  setup do
    @redis = FakeRedis.new
    @file_store = CsvDrop::Stores::DiskFileStore.new
    @store = CsvDrop::Stores::RedisSessionStore.new(file_store: @file_store, redis: @redis)
  end
